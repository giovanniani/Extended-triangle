...
*/
   ...
   * @param binding 
   */
  public void exportScopeRecursive(Declaration binding) {
    Identifier i = null;

    if (binding instanceof ProcDeclaration) {
      i = ((ProcDeclaration) binding).I;
    } else if (binding instanceof FuncDeclaration) {
      i = ((FuncDeclaration) binding).I;
    } else if (binding instanceof SequentialDeclaration) {
      exportScopeRecursive( ( (SequentialDeclaration) binding ).D1 );
      exportScopeRecursive( ( (SequentialDeclaration) binding ).D2 );
    } else if (binding instanceof RecursiveDeclaration){
      exportScopeRecursive( ( (RecursiveDeclaration) binding ).D );
    } else {
      reporter.reportError ("Is not a CONST or VAR identifier. " + binding.toString(), "", binding.position);
    }
